...
export default function ContactPage() {
  ...
  async function onSubmit(dto: ContactDto) {
    setIsLoading(true);
    // Demo only: no backend is connected, so this simulates a submission
    // instead of sending the message anywhere.
    await new Promise((resolve) => setTimeout(resolve, 500));
    console.info("Contact form submitted (demo, not sent):", dto);
    toast({ title: "Message sent!", description: "This is a demo — your message wasn't actually delivered anywhere." });
    reset();
    setIsLoading(false);
  }
