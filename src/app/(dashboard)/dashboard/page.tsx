  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Welcome back, {DEMO_USER.name.split(" ")[0]}</h1>
          <p className="text-sm text-muted-foreground mt-1">Here&apos;s what&apos;s happening with your resumes.</p>
        </div>
        <Button asChild>
          <Link href="/dashboard/resumes"><Plus className="mr-2 h-4 w-4" />New Resume</Link>
        </Button>
      </div>
        ...
        <Card>
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold mt-1">{totalResumes}</p>
            </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button variant="ghost" size="sm" asChild>
            <Link href="/dashboard/resumes">View all <ArrowRight className="ml-1.5 h-3.5 w-3.5" /></Link>
          </Button>
        </CardHeader>
        <CardContent>
          {recentResumes.length === 0 ? (
            <EmptyState
              ...
              description="Create your first resume to get started with the AI-powered writing tools."
              action={<Button asChild><Link href="/dashboard/resumes"><Plus className="mr-2 h-4 w-4" />Create Resume</Link></Button>}
            />
          ) : (
        ...
        <Link href="/dashboard/resumes">
          <Card className="hover:shadow-md transition-shadow h-full">
            <CardContent className="p-6">
              ...
              <p className="text-sm text-muted-foreground">Download as PDF, DOCX or JSON</p>
            </CardContent>
          </Card>
        </Link>
      </div>
