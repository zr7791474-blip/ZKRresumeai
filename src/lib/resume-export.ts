...
type ExportableResume = Pick<ResumeWithSections, "title" | "data" | "sections">;
...
async function buildResumePdfBytes(resume: ExportableResume): Promise<Uint8Array> {
...
async function buildResumeDocxBlob(resume: ExportableResume): Promise<Blob> {
  const children: Paragraph[] = [
    new Paragraph({ text: resume.title, heading: HeadingLevel.TITLE }),
  ];
...
function buildResumeJsonBlob(resume: ExportableResume): Blob {
  return new Blob([JSON.stringify({ title: resume.title, data: resume.data, sections: resume.sections }, null, 2)], {
    type: "application/json",
  });
}

function slugify(text: string): string {
  return text.toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "resume";
}
...
/** Builds the requested export format entirely in the browser and triggers a download. */
export async function exportResume(resume: ExportableResume, format: "PDF" | "DOCX" | "JSON"): Promise<void> {
  const filename = slugify(resume.title);

  if (format === "PDF") {
    const bytes = await buildResumePdfBytes(resume);
    triggerDownload(new Blob([bytes as BlobPart], { type: "application/pdf" }), `${filename}.pdf`);
    return;
  }
  ...
  triggerDownload(buildResumeJsonBlob(resume), `${filename}.json`);
}
